  ...
  if (globalStore[username]) {
    try {
      ...
      if (result) {
        ...
        console.log(
          "\n❌ Invalid password for user " + username + ". Please try again.\n"
        );
    ...
    // Tell the user they can't log in to a non-existent account
...
hashPassword = async (username, password) => {
  try {
    ...
    console.log(
      `Password for user ${username} hashed successfully. Hash: ${hashedPassword}`
    );
  } catch (error) {
